...
[RHO](#rho) = water vapor density measurement
[$\mathrm{g\ }\mathrm{m}^{-3}$]; only one is used in any calculation  
[ATX](#ambient-t) = reference ambient temperature [$^{\circ}C$]    
$T_{K}$=ATX+$T_{0}$\ $^{\dagger}$ = ambient temperature [K]   
$p$ = [PSXC](#psx) = reference ambient pressure [hPa]  
$p_{h}$ = [CAVP](#p-special)\_x = pressure in instrument "x" housing [hPa]  
$e_{t}$ = intermediate vapor pressure used for calculation only  
$e$ = [EWx](#ewx) = water vapor pressure from source x [hPa]  
...
\ \ \ \ \ \ \ \ obtain $e_{t}$ from \@ref(eq:MKi) using $T_{FP}$=MIRRTMP_DPx + $T_{0}$  
\ \ \ \ else (i.e., MIRRTMP_DPx $\geq 0^\circ$C):  
\ \ \ \ \ \ \ \ obtain $e_{t}$ from \@ref(eq:MKw)
using $T_{DP}=\mathrm{MIRRTMP\_DPx}+T_{0}$  
...
(\#eq:ebox)
\end{equation}
\ \ \ \ obtain DP_DPx by finding the dew point corresponding to the
vapor pressure $e$:  
\begin{equation}
\mathrm{\{DP_DPx\}} = F_{d}(e) 
(\#eq:DPxCbox)
\end{equation}
- - - - - - - - - - - - - - - - - - - -    
